...
- React/TS Clean Code: `~/.codex/skills/react-typescript-clean-code/SKILL.md`
- React Native/Expo: `~/.codex/skills/react-native-architecture/SKILL.md`
- Backend Architecture: `~/.codex/skills/backend-architecture/SKILL.md` (only if task touches backend)
- Testing Strategy: `~/.codex/skills/testing-strategy/SKILL.md` (only if writing tests)
...
- **Semantic Memory**: before a non-trivial change run `cogni search "<tags>"` (CLI, via bash) and after closing it run `cogni save` with a `topic_key` in the form `<domain>/<subdomain>/<topic>`. Rules: `~/.codex/rules/engineering-invariants.md`;
  full protocol: `~/.codex/skills/cogni/SKILL.md`.
