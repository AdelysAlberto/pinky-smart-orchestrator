...
- Testing Standards: `~/.codex/skills/testing-strategy/SKILL.md` (only if verifying tests)
...
- **Semantic Memory**: when a review closes with a finding worth remembering (a recurring anti-pattern, an invariant that was not obvious), record it with `cogni save` and a `topic_key` in the form `<domain>/<subdomain>/<topic>`. Protocol: `~/.codex/skills/cogni/SKILL.md`.
