...
- React/TS Clean Code: `config/skills/react-typescript-clean-code/SKILL.md`
- React Native/Expo: `config/skills/react-native-architecture/SKILL.md`
- Backend Architecture: `config/skills/backend-architecture/SKILL.md` (only if task touches backend)
- Testing Strategy: `config/skills/testing-strategy/SKILL.md` (only if writing tests)
...
- **Semantic Memory**: before a non-trivial change run `cogni search "<tags>"` (CLI, via bash) and after closing it run `cogni save` with a `topic_key` in the form `<domain>/<subdomain>/<topic>`. Rules: `config/rules/engineering-invariants.rules.md`;
  full protocol: `config/skills/cogni/SKILL.md`.
