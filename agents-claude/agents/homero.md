...
- React/TS Clean Code: `~/.claude/skills/react-typescript-clean-code/SKILL.md`
- React Native/Expo: `~/.claude/skills/react-native-architecture/SKILL.md`
- Backend Architecture: `~/.claude/skills/backend-architecture/SKILL.md` (only if task touches backend)
- Testing Strategy: `~/.claude/skills/testing-strategy/SKILL.md` (only if writing tests)
...
- **Semantic Memory**: before a non-trivial change run `cogni search "<tags>"` (CLI, via bash) and after closing it run `cogni save` with a `topic_key` in the form `<domain>/<subdomain>/<topic>`. Rules: `~/.claude/rules/engineering-invariants.md`;
  full protocol: `~/.claude/skills/cogni/SKILL.md`.
