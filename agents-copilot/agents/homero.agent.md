...
- React/TS Clean Code: `~/.copilot/skills/react-typescript-clean-code/SKILL.md`
- React Native/Expo: `~/.copilot/skills/react-native-architecture/SKILL.md`
- Backend Architecture: `~/.copilot/skills/backend-architecture/SKILL.md` (only if task touches backend)
- Testing Strategy: `~/.copilot/skills/testing-strategy/SKILL.md` (only if writing tests)
...
- **Semantic Memory**: before a non-trivial change run `cogni search "<tags>"` (CLI, via bash) and after closing it run `cogni save` with a `topic_key` in the form `<domain>/<subdomain>/<topic>`. Rules: `~/.copilot/instructions/engineering-invariants.md`;
  full protocol: `~/.copilot/skills/cogni/SKILL.md`.
